...
module.exports = function(roto) {
	...
	roto.defineTask('dir-copy', function(callback, options, target, globalOptions) {
		...
		// perform copy
		console.log(roto.colorize('cp: ', 'gray') + options.from + roto.colorize(' -> ', 'gray') + options.to);
		dir_copy(options, function(err) {
			if (err) {
				console.error(roto.colorize('ERROR:', 'red') + ' ' + err);
				return callback(false);
	...
	roto.defineTask('dir-move', function(callback, options, target, globalOptions) {
		...
		// perform move
		console.log(roto.colorize('mv: ', 'gray') + options.from + roto.colorize(' -> ', 'gray') + options.to);
	...
	roto.defineTask('dir-remove', function(callback, options, target, globalOptions) {
		...
		// validate options
		if (!options.path) {
			console.error(roto.colorize('ERROR:', 'red') + ' No "path" provided.');
			return callback(false);
		}

		// perform remove
		console.log(roto.colorize('rm: ', 'gray') + options.path);
