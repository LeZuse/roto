...
module.exports = function(roto) {
	...
	roto.defineTask('png', function(callback, options, target, globalOptions) {
		...
		if (options.compressor === 'none') {
			console.error(roto.colorize('ERROR: ', 'red') + 'You need to specify a compressor!');
			return callback(false);
		}
		binary = options.binary || binaries[options.compressor];
		...
		// queue files
		console.log('Compressing pngs using ' + options.compressor + '...');
		queue = async.queue(function(filename, callback) {
			console.log(roto.colorize('   + ', 'gray') + filename);
			...
			proc.stderr.on('data', function (data) {
				if (/^execvp\(\)/.test(data)) {
					console.error(roto.colorize('Error: ', 'red') + ' Failed to start ' + binary);
				}
			});
		}, options.workers);
