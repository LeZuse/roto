...
module.exports = function(roto) {
	...
	roto.defineTask('uglify', function(callback, options, target, globalOptions) {
		...
		console.log(colorize(options.output, 'white'));
		...
		for (i = 0; i < paths.length; i++) {
			...
			console.log(colorize('   + ', 'gray') + paths[i]);
		...
		try {
			...
			console.error(colorize('ERROR:', 'red') + ' Unable to parse source.');
			...
			if (typeof e.line !== 'undefined') {
				...
				for (i = start; i <= end; i++) {
					console.error(colorize(lines[i], i === line ? 'red' : 'gray'));
				}
		...
		try {
			...
			if (filesize_input > 0) {
				console.log(colorize('   ' + roto.formatFilesize(filesize_output) + ' (' + Math.round((filesize_input - filesize_output) / filesize_input * 100, 1) + '% reduction)', 'gray'));
			}
		} catch(err) {
			console.error(colorize('ERROR:', 'red') + ' Unable write output (' + options.output + ').');
			return callback(false);
		}
