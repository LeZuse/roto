...
module.exports = function(roto) {
	...
	roto.defineTask('s3', function(callback, options, target, globalOptions) {
		...
		console.log(colorize(folder, 'white') + ' -> ' + colorize(options.bucket, 'white') + '/' + folder_remote.replace(/^\//, '') + ' (S3)');
		...
		async.series([
			...
			function(callback) {
				...
				client.getFile(folder_remote + options.manifest, function(err, res) {
					if (res.statusCode === 200) {
						...
						console.log(' (empty) [' + colorize('success', 'green') + ']');
						callback();
					}
				});
			},
			...
			function(callback) {
				...
				for (var i = 0, n = options.ignore.length; i < n; i++) {
					patterns_ignore.push(folder + '**/' + options.ignore);
				}
			...
			console.log(delta.length.toString() + ' files queued for upload (' + roto.formatFilesize(bytes_queued) + ')');
			...
			var interval_display = setInterval(function() {
				process.stdout.write('\r' + colorize(Math.round(bytes_uploaded / bytes_queued * 100).toString() + '%', 'white') + ' - ' + roto.formatFilesize(bytes_uploaded) + ' of ' + roto.formatFilesize(bytes_queued) + '              ');
			}, 1000);
			...
			queue_files.drain = function(err) {
				...
				updateManifest(function() {
					if (err) {
						console.error(err + ' [' + colorize('fail', 'red') + ']');
						callback(false);
					} else {
						console.log('[' + colorize('success', 'green') + ']');
						callback();
