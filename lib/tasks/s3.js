...
module.exports = function(roto) {
	...
	roto.defineTask('s3', function(callback, options, target, globalOptions) {
		...
		async.series({
			...
			console.log(delta.length.toString() + ' files queued for upload (' + (bytes_queued / 1048576).toFixed(2) + 'mb)');
			...
			var interval_display = setInterval(function() {
				process.stdout.write('\r' + colorize(Math.round(bytes_uploaded / bytes_queued * 100).toString() + '%', 'white') + ' - ' + Math.round(bytes_uploaded / 1024) + 'kb of ' + Math.round(bytes_queued / 1024) + 'kb            ');
			}, 1000);
